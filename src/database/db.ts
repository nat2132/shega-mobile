  ...
  try {
    ...
    const sales = database.getAllSync(`
      SELECT 
        ...
        CASE 
          WHEN s.batchId IS NOT NULL THEN (
            SELECT GROUP_CONCAT(i4.image, '|')
            FROM sales sub4
            JOIN items i4 ON sub4.itemId = i4.id AND i4.businessId = sub4.businessId
            WHERE sub4.batchId = s.batchId AND sub4.businessId = s.businessId
              AND i4.image IS NOT NULL AND i4.image != ''
          )
          ELSE i.image 
        END as images,
        CASE 
    ...
    const adjustments = database.getAllSync(`
      SELECT 'adjustment' as type, 'adjustment' as category, a.id, a.newValue as value, a.quantity, a.type as adjType, a.oldValue, COALESCE(i.name, 'Item') as label, a.createdAt, i.basePurchasePrice, i.image as images, au.name as userName, au.avatar as userAvatar
      FROM adjustments a
    ...
    const inventory = database.getAllSync(`
      SELECT 'inventory' as type, 'inventory' as category, sm.id, (sm.quantityAdded * i.basePurchasePrice) as value, sm.quantityAdded as quantity, i.name as label, sm.createdAt, i.companyName, i.image as images, mu.name as userName, mu.avatar as userAvatar
    ...
    const combined = [...sales, ...adjustments, ...inventory, ...deletions]
      .map((item: any) => {
        if (typeof item?.images === 'string') {
          item.images = item.images.split('|').filter((s: string) => s && s.trim().length > 0);
...
/**
 * FACTORY RESET (dev/testing) — wipe EVERYTHING for a brand-new-install state:
 * all businesses/users/products/sales/customers/debts, sync queues, history,
 * device identity + pairing, and app settings. The schema is preserved; the
 * app re-initializes it on next launch exactly like a first install.
 */
export const factoryResetDatabase = (): boolean => {
  try {
    const database = getDB();
    const tables = database.getAllSync(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE 'android_%' AND name NOT LIKE 'ios_%'"
    ) as any[];
    database.execSync('PRAGMA foreign_keys = OFF;');
    database.execSync('BEGIN TRANSACTION;');
    try {
      for (const t of tables) {
        try { database.execSync(`DELETE FROM ${t.name};`); } catch { /* views/protected */ }
      }
      database.execSync('DELETE FROM sqlite_sequence;');
      database.execSync('COMMIT;');
    } catch (e) {
    ...
    console.log('[FactoryReset] all local data cleared — fresh-install state');
    return true;
  } catch (error) {
...
export const getReturnsBySaleId = (saleId: number) => {
  try {
    ...
    if (bizId == null) return [];
    return database.getAllSync(`
      SELECT r.*, i.name as itemName, i.baseUnit, i.image
      FROM returns r
