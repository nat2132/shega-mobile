...
const TimeSystemSettings = () => {
    ...
    return (
      <TouchableOpacity
        style={[
          ...
          isSelected && [styles.selectedCard, { borderColor: G.fg, backgroundColor: G.bgCard }],
        ]}
      ...
      >
        <View style={styles.cardLeft}>
          <View style={[styles.iconBox, { backgroundColor: G.bg }]}>
            <Clock size={22} color={isSelected ? G.fg : G.fgSecondary} />
          </View>
          <View style={{ flex: 1 }}>
            <AppText variant="title-sm" weight="bold" style={[styles.cardTitle, { color: G.fg }]} numberOfLines={1}>{title}</AppText>
            <AppText variant="body-sm" weight="medium" style={[styles.previewText, { color: G.fgSecondary }]} numberOfLines={2}>{preview}</AppText>
          </View>
        </View>
        {isSelected ? (
  ...
  return (
    <View style={[styles.container, { backgroundColor: G.bg }]}>
      ...
      <AppText variant="caption" weight="bold" transform="uppercase" style={[styles.headerLabel, { color: G.fgSecondary }]} numberOfLines={2}>{t('settings.time_system_settings')}</AppText>
      <AppText variant="display" weight="bold" style={[styles.mainTitle, { color: G.fg }]} numberOfLines={2}>{t('settings.time_format')}</AppText>
      <AppText variant="body-sm" weight="medium" style={[styles.subtitle, { color: G.fgSecondary }]} numberOfLines={3}>{t('settings.time_format_desc')}</AppText>
      ...
      <AppText variant="caption" weight="medium" style={[styles.note, { color: G.fgSecondary, textAlign: 'center', marginTop: 10 }]} numberOfLines={2}>
