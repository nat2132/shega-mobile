    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={modalStyles.overlay}>
        ...
        <View style={[modalStyles.sheet, { backgroundColor: colors.background }]}>
          ...
          <AppText variant="title" weight="bold" style={[modalStyles.title, { color: colors.text }]} numberOfLines={2}>{t('dash.settle_credit')}</AppText>
          <AppText variant="body-sm" weight="medium" style={[modalStyles.itemName, { color: colors.textSecondary }]} numberOfLines={1}>{item.name}</AppText>

          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 12, paddingBottom: 20 }}>
            {/* Full Payment Option */}
            <TouchableOpacity
              style={[modalStyles.option, { borderColor: paymentType === 'full' ? colors.text : colors.border, backgroundColor: colors.card }]}
              onPress={() => setPaymentType('full')}
            >
              <View style={[modalStyles.radio, { borderColor: paymentType === 'full' ? colors.text : colors.border, backgroundColor: paymentType === 'full' ? colors.text : 'transparent' }]} />
              <View style={{ flex: 1 }}>
                <AppText variant="body" weight="bold" style={[modalStyles.optionLabel, { color: colors.text }]} numberOfLines={1}>{t('dash.paid_full')}</AppText>
                <AppText variant="caption" weight="medium" style={[modalStyles.optionSub, { color: colors.textSecondary }]} numberOfLines={2}>{t('dash.settle_entire')}</AppText>
              </View>
              <AppText variant="body" weight="bold" shrink={false} style={[modalStyles.optionPrice, { color: colors.text }]} numberOfLines={1}>{totalCredit.toLocaleString()} {t('common.etb')}</AppText>
            </TouchableOpacity>

            {/* Partial Payment Option */}
            <TouchableOpacity
              style={[modalStyles.option, { borderColor: paymentType === 'partial' ? colors.text : colors.border, backgroundColor: colors.card }]}
              onPress={() => setPaymentType('partial')}
            >
              <View style={[modalStyles.radio, { borderColor: paymentType === 'partial' ? colors.text : colors.border, backgroundColor: paymentType === 'partial' ? colors.text : 'transparent' }]} />
              <View style={{ flex: 1 }}>
                <AppText variant="body" weight="bold" style={[modalStyles.optionLabel, { color: colors.text }]} numberOfLines={1}>{t('dash.partial_payment')}</AppText>
                <AppText variant="caption" weight="medium" style={[modalStyles.optionSub, { color: colors.textSecondary }]} numberOfLines={2}>{t('dash.record_fractional')}</AppText>
              </View>
            </TouchableOpacity>

            {/* Partial Amount Input */}
            {paymentType === 'partial' && (
              <View style={[modalStyles.inputContainer, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <TextInput
                ...
                />
                <AppText variant="body" weight="bold" shrink={false} style={[modalStyles.inputUnit, { color: colors.textSecondary }]} numberOfLines={1}>{t('common.etb')}</AppText>
              </View>
            )}

            {/* Confirm Button */}
            <TouchableOpacity
              style={[modalStyles.confirmBtn, { backgroundColor: colors.text }]}
              onPress={handleConfirm}
            >
              <AppText variant="body" weight="bold" shrink={false} style={[modalStyles.confirmBtnText, { color: colors.background }]} numberOfLines={1}>{t('dash.confirm_payment')}</AppText>
            </TouchableOpacity>
          </ScrollView>
        </View>
      </View>
  ...
  return (
    <View style={[detailStyles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={detailStyles.header}>
        <TouchableOpacity onPress={onBack} style={[detailStyles.backBtn, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <ChevronLeft size={22} color={colors.text} />
        </TouchableOpacity>
        <View style={{ flex: 1, marginLeft: 14 }}>
          <AppText variant="micro" weight="bold" transform="uppercase" style={[detailStyles.headerSub, { color: colors.textSecondary }]} numberOfLines={1}>
            {t('dash.sourcing')}
          </AppText>
          <AppText variant="title" weight="bold" style={[detailStyles.headerTitle, { color: colors.text }]} numberOfLines={2}>
            {t('dash.asset_origin')}
          </AppText>
        </View>
      </View>
        ...
        <Animated.View entering={FadeInDown.duration(500)}>
          ...
          <View style={[detailStyles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={detailStyles.cardHeader}>
              <View style={[detailStyles.iconBox, { backgroundColor: colors.text + '08' }]}>
                <Building2 size={28} color={colors.text} />
              </View>
              <View style={{ flex: 1, marginLeft: 14 }}>
                <AppText variant="title" weight="bold" style={[detailStyles.itemName, { color: colors.text }]} numberOfLines={2}>
                  {item.name}
                </AppText>
                <AppText variant="body-sm" weight="medium" style={[detailStyles.itemSub, { color: colors.textSecondary }]} numberOfLines={1}>
            ...
            {/* Purchase Details */}
            <AppText variant="micro" weight="bold" transform="uppercase" style={[detailStyles.sectionLabel, { color: colors.textSecondary }]} numberOfLines={1}>
            ...
            <AppRow
              label={t('dash.pack_purchase_price')}
              value={`${(item.packPurchasePrice || 0).toLocaleString()} ${t('common.etb')}`}
              valueVariant="body"
              valueWeight="bold"
              labelMaxLines={2}
              style={{ marginBottom: Spacing.sm }}
            ...
            <AppRow
              label={t('dash.base_purchase_price')}
              value={`${(item.basePurchasePrice || 0).toLocaleString()} ${t('common.etb')}`}
              valueVariant="body"
              valueWeight="bold"
              labelMaxLines={2}
              style={{ marginBottom: Spacing.sm }}
            ...
            {/* Settle Button */}
            <TouchableOpacity
              style={[detailStyles.settleBtn, { backgroundColor: colors.success || '#34C759' }]}
            ...
            <View style={detailStyles.cardFooter}>
              <View>
                <AppText variant="caption" weight="medium" style={[detailStyles.footerLabel, { color: colors.textSecondary }]} numberOfLines={1}>
                  {t('dash.supplier_interface')}
                </AppText>
                <AppText variant="title-sm" weight="bold" style={[detailStyles.footerValue, { color: colors.text }]} numberOfLines={1}>
                  {item.supplierPhone || 'N/A'}
                </AppText>
              </View>
              {item.supplierPhone ? (
                <TouchableOpacity style={[detailStyles.callBtn, { backgroundColor: colors.text }]}>
                  <Phone size={18} color={colors.background} />
                  <AppText variant="body-sm" weight="bold" shrink={false} style={[detailStyles.callBtnText, { color: colors.background }]} numberOfLines={1}>
                    {t('dash.call')}
                  </AppText>
                </TouchableOpacity>
              ) : null}
    ...
    <View style={[listStyles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={listStyles.headerNode}>
          <AppText variant="micro" weight="bold" transform="uppercase" style={[listStyles.headerSub, { color: colors.textSecondary }]} numberOfLines={1}>
            {t('dash.supply_intel')}
          </AppText>
          <AppText variant="display" weight="bold" style={[listStyles.headerTitle, { color: colors.text }]} numberOfLines={2}>
          ...
          return (
            <Animated.View key={item.id} entering={FadeInDown.delay(idx * 50).duration(500)}>
              ...
              >
                <AppListItem
                  left={
                    <View style={[listStyles.iconBox, { backgroundColor: colors.text + '06' }]}>
                  ...
                  right={
                    <View style={listStyles.statArea}>
                      <AppText variant="body" weight="bold" shrink={false} style={[listStyles.qtyText, { color: colors.text }]} numberOfLines={1}>
                        {(item.totalBaseQuantity || 0).toLocaleString()}
                        <AppText variant="caption" weight="medium" style={listStyles.unitSmall}> {item.baseUnit || 'pcs'}</AppText>
                      </AppText>
                      {isOverdue ? (
                        <View style={[listStyles.badge, { backgroundColor: '#FF3B3015' }]}>
                          <AlertTriangle size={10} color="#FF3B30" />
                          <AppText variant="micro" weight="bold" transform="uppercase" shrink={false} style={[listStyles.badgeText, { color: '#FF3B30' }]} numberOfLines={1}>
                            {t('dash.overdue')}
                          </AppText>
                        </View>
                      ) : (
                ...
                <View style={[listStyles.cardFooter, { backgroundColor: colors.text + '03', borderTopColor: colors.border }]}>
                  <View style={listStyles.creditFooterRow}>
                    <CreditCard size={13} color={colors.textSecondary} />
                    <AppText variant="caption" weight="medium" style={[listStyles.creditFooterLabel, { color: colors.textSecondary }]} numberOfLines={1}>
                      {t('dash.total_credit')}
                    </AppText>
                    <AppText variant="body-sm" weight="bold" shrink={false} style={[listStyles.creditFooterAmount, { color: '#FF3B30' }]} numberOfLines={1}>
                      {((item.packPurchasePrice || 0) * (item.totalPackQuantity || 1) || (item.basePurchasePrice || 0) * (item.totalBaseQuantity || 1) || 0).toLocaleString()} {t('common.etb')}
                    </AppText>
                  </View>
        ...
        {items.length === 0 && (
          <View style={listStyles.empty}>
            <View style={[listStyles.emptyIcon, { backgroundColor: (colors.success || '#34C759') + '15' }]}>
              <Check size={36} color={colors.success || '#34C759'} />
            </View>
            <AppText variant="title" weight="bold" style={[listStyles.emptyTitle, { color: colors.text }]} numberOfLines={2}>
              {t('dash.zero_credit')}
            </AppText>
            <AppText variant="body" weight="medium" style={[listStyles.emptySub, { color: colors.textSecondary }]} numberOfLines={3}>
...
const detailStyles = StyleSheet.create({
  ...
  card: { borderRadius: 28, borderWidth: 1, padding: 22 },
  ...
  settleBtnText: { fontSize: 15, fontFamily: Fonts.bold, color: '#FFF' },
